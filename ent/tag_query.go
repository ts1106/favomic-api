...
func (tq *TagQuery) QueryComics() *ComicQuery {
	query := &ComicQuery{config: tq.config}
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withComics: tq.withComics.Clone(),
...
func (tq *TagQuery) WithComics(opts ...func(*ComicQuery)) *TagQuery {
	query := &ComicQuery{config: tq.config}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withComics; query != nil {
		if err := tq.loadComics(ctx, query, nodes,
			func(n *Tag) { n.Edges.Comics = []*Comic{} },
			func(n *Tag, e *Comic) { n.Edges.Comics = append(n.Edges.Comics, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadComics(ctx context.Context, query *ComicQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Comic)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.ComicsTable)
		s.Join(joinT).On(s.C(comic.FieldID), joinT.C(tag.ComicsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.ComicsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.ComicsPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "comics" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
