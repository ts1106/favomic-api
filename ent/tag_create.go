...
func (tc *TagCreate) AddComicIDs(ids ...uuid.UUID) *TagCreate {
	tc.mutation.AddComicIDs(ids...)
	return tc
}

// AddComics adds the "comics" edges to the Comic entity.
func (tc *TagCreate) AddComics(c ...*Comic) *TagCreate {
	ids := make([]uuid.UUID, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return tc.AddComicIDs(ids...)
}
...
func (tc *TagCreate) createSpec() (*Tag, *sqlgraph.CreateSpec) {
	...
	if nodes := tc.mutation.ComicsIDs(); len(nodes) > 0 {
