...
// The MagazineFunc type is an adapter to allow the use of ordinary
// function as Magazine mutator.
type MagazineFunc func(context.Context, *ent.MagazineMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f MagazineFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.MagazineMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.MagazineMutation", m)
	}
	return f(ctx, mv)
}
