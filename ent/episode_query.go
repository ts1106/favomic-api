...
func (eq *EpisodeQuery) QueryComic() *ComicQuery {
	query := &ComicQuery{config: eq.config}
...
func (eq *EpisodeQuery) WithComic(opts ...func(*ComicQuery)) *EpisodeQuery {
	query := &ComicQuery{config: eq.config}
...
func (eq *EpisodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Episode, error) {
	...
	if query := eq.withComic; query != nil {
		if err := eq.loadComic(ctx, query, nodes, nil,
			func(n *Episode, e *Comic) { n.Edges.Comic = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EpisodeQuery) loadComic(ctx context.Context, query *ComicQuery, nodes []*Episode, init func(*Episode), assign func(*Episode, *Comic)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "comic_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
