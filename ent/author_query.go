...
func (aq *AuthorQuery) QueryComics() *ComicQuery {
	query := &ComicQuery{config: aq.config}
...
func (aq *AuthorQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (aq *AuthorQuery) Clone() *AuthorQuery {
	...
	return &AuthorQuery{
		...
		withComics: aq.withComics.Clone(),
...
func (aq *AuthorQuery) WithComics(opts ...func(*ComicQuery)) *AuthorQuery {
	query := &ComicQuery{config: aq.config}
...
func (aq *AuthorQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Author, error) {
	...
	if query := aq.withComics; query != nil {
		if err := aq.loadComics(ctx, query, nodes,
			func(n *Author) { n.Edges.Comics = []*Comic{} },
			func(n *Author, e *Comic) { n.Edges.Comics = append(n.Edges.Comics, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AuthorQuery) loadComics(ctx context.Context, query *ComicQuery, nodes []*Author, init func(*Author), assign func(*Author, *Comic)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Author)
	for i := range nodes {
	...
	query.Where(predicate.Comic(func(s *sql.Selector) {
		s.Where(sql.InValues(author.ComicsColumn, fks...))
	}))
